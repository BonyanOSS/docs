...
description: "How Etha3a API keeps one client contract while upstream providers fail, time out, or return empty data."
---

Etha3a API depends on public upstream providers, and public upstream providers
...
The important idea is simple: **clients integrate with Etha3a, not with every
upstream provider.**
...
If you fork or self-host Etha3a API, provider priority lives in each module's
...
<AccordionGroup>
  <Accordion title="Does fallback retry upstream 429 responses?">
    ...
    so the next source is tried. A 429 returned to your client is Etha3a API's
    own rate limit.
  </Accordion>
